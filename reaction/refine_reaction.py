...
parser.add_argument('--batch_time', type=int, default=5) # case1 40
parser.add_argument('--rand_coef', type=float, default=0.0)
parser.add_argument('--batch_size', type=int, default=1)
parser.add_argument('--batch_sample', type=int, default=4)
...
vdata = np.loadtxt('./data/vdata3.txt')
...
def reodeint(func, true_y0, vt, ode_nums):
    result_y = torch.cat([odeint(func, true_y0[i], vt[i]) for i in range(args.batch_size)])
    return result_y
...
if args.viz:
    makedirs('png_refine{}'.format(args.CASE))
    import matplotlib.pyplot as plt
...
def visualize(vt, true_y, pred_y, odefunc, itr, loss):
    ...
    if args.viz:
        ...
        fig.tight_layout()
        plt.savefig('png_refine{}/{}.jpg'.format(args.CASE,itr))
        #plt.draw()
        #plt.pause(0.001)
...
[[0.32894366979599, -0.3290727436542511], [-0.08368273079395294, 0.08375126868486404], [-0.6211790442466736, 0.6209065318107605], [0.0, 0.0], [0.09656161814928055, -0.09656648337841034], [0.09018805623054504, -0.09044774621725082]])
...
def saveStateDict(input,itr,CASE):
    """A function used to save model.state_dict() OrderedDict as json file with 'matrix%s.json'%itr
    file name.
    ...
    if not os.path.exists('matrix_refine{}'.format(CASE)):
        os.makedirs('matrix_refine{}'.format(CASE))
    with open('matrix_refine{}/matrix_refine{}.json'.format(CASE,itr), 'w') as fp:
    ...
    for itr in range(1, args.niters + 1):
        ...
        if itr % args.test_freq == 0:
            with torch.no_grad():
                ...
                pred_train = reodeint(func, true_y0, vt, args.ode_nums)
                ...
                np.savetxt('loss_train_refine{}.txt'.format(args.CASE), loss_train_list)
...
loss_data=np.loadtxt('loss_train_refine{}.txt'.format(args.CASE))
plt.figure()
plt.plot(loss_data[:,0],loss_data[:,1])
plt.xlabel('itr')
plt.ylabel('total train set loss')
plt.savefig('loss_train_refine{}.jpg'.format(args.CASE))
