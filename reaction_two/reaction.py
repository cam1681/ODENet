...
equationdata = np.loadtxt('./data/vdata3.txt')
...
true_y1 = torch.from_numpy(equationdata[:,2].reshape(-1,1,1)).type(torch.FloatTensor)
...
class OdeNet(nn.Module):
    def __init__(self, variable_nums, ode_nums, basis_order, vc_init_flag, train_size, true_ym):
        ...
        self.true_y2 = nn.Parameter(torch.Tensor(train_size, 1, ode_nums))
        self.err.data.uniform_(-0, 0)
        self.true_y2.data = true_ym
    ...
    def forward(self, t, x_input):
        ...
        if x_input.dim() == 2:
            output = torch.mm(_compute_theta2d(),torch.stack([-10**self.vc[:,0],10**self.vc[:,1]],dim=1))
        else:
            #output = torch.matmul(_compute_theta3d(),10**self.vc)
            output = torch.mm(_compute_theta3d(),torch.stack([-10**self.vc[:,0],10**self.vc[:,1]],dim=1))
...
if __name__ == '__main__':
    ...
    func.float()
    true_y = update_true_y(true_y1, func.true_y2)
    ...
    optimizer = optim.Adam(\
        ...
        {"params": func.true_y2, "lr": args.lr},\
    ...
    for itr in range(1, args.niters + 1):
        ...
        l1_regularization = l1_regularization + torch.norm(func.vc,1) + torch.norm(func.err,1)
        factor =  0.0000
        ...
        if itr % args.test_freq == 0:
            with torch.no_grad():
                ...
                loss_train = torch.mean(torch.abs(pred_train[:,:,0] - train_y[:,:,0]))
                ...
                func.true_y2.data = pred_train[:,0,1].reshape(-1,1,1)
                true_y = update_true_y(true_y1, func.true_y2)
                true_y0 = true_y[indexint]
                train_y = true_y[:args.train_size]
