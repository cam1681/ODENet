...
equationdata = np.loadtxt('./data/actin5p.txt',delimiter=',')
...
class OdeNet(nn.Module):
    def __init__(self, variable_nums, ode_nums, basis_order, vc_init_flag, train_size, true_ym):
        ...
        self.vc = nn.Parameter(torch.Tensor(2,4))
        ...
        self.vc.data[0, 1] = 0.0011
        self.vc.data[0, 3] = 0.008
        self.vc.data[1, 0] = -2.2982
        self.vc.data[1, 2] = 1.5906
        self.vc.data[1, 3] = 2*0.008


    def forward(self, t, x_input):
        if x_input.dim() == 2:
            _dotx = torch.zeros(x_input.size(0),3)
            _dotx[:,0] = self.vc[0,0] + self.vc[0,1]*x_input[:,1] + self.vc[0,2]*x_input[:,2] + self.vc[0,3]*x_input[:,2]*x_input[:,2]
            _dotx[:,1] = self.vc[1,0]*x_input[:,0] + self.vc[1,1]*x_input[:,1] + self.vc[1,2]*x_input[:,0]*x_input[:,2] + self.vc[1,3]*x_input[:,2]*x_input[:,2]
            _dotx[:,2] = - _dotx[:,1]
            output = _dotx
        else:
            _dotx = torch.zeros(x_input.size(0),1,3)
            _dotx[:,0,0] = self.vc[0,0] + self.vc[0,1]*x_input[:,0,1] + self.vc[0,2]*x_input[:,0,2] + self.vc[1,3]*x_input[:,0,2]*x_input[:,0,2]
            _dotx[:,1] = self.vc[1,0]*x_input[:,0,0] + self.vc[1,1]*x_input[:,0,1] + self.vc[1,2]*x_input[:,0,0]*x_input[:,0,2] + self.vc[1,3]*x_input[:,0,2]*x_input[:,0,2]
            _dotx[:,0,2] = - _dotx[:,0,1]
            output = _dotx
...
if __name__ == '__main__':
    ...
    for itr in range(1, args.niters + 1):
        ...
        #loss_y1 = torch.where(pred_y[:,:,:,0] < 0, torch.abs(pred_y), torch.full_like(pred_y, 0))

        #loss_y1 = torch.mean(
            #torch.where(pred_y[:, :, :, 0] < 0, torch.abs(pred_y[:, :, :, 0]), torch.full_like(pred_y[:, :, :, 0], 0)))
        #loss += loss_y1
